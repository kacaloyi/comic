...
class AdminController extends Controller
{
    ...
    public function get_short_url(){
	    ...
	    //die ($url);// $url必须是base64格式的。在网页中用window.btoa转换普通url到base64格式。 
		//$url2=file_get_contents("http://api.t.sina.com.cn/short_url/shorten.json?source=3271760578&url_long=".urlencode($url));
		//sina的短链接不能这样去获取，只能直接跳转过去用了。
		file_get_contents("https://sina.lt/index.html");
		$url2=file_get_contents("https://sina.lt/api.php?from=w&site=dwz.date&url=".$url);//file_get_contents("https://sina.lt/api.php?from=w&site=dwz.date&url=".$url);

		//$json = json_decode($url2);
	//	$zl=$json['result'];//['short_url'];//->url_short;
		$this->assign('str',$url2);
		$this->display();
