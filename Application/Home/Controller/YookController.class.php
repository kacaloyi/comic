...
class YookController extends HomeController {
		    ...
		    if($list){
			   ...
			   foreach($list as $v){
				    ...
				    if(!$money || $money<0 || $money == 0){
					 ...
					 $read = M('read_charge')->where(array('user_id'=>$this->user['id'],'yid'=>$yid,'ji_no'=>$v['ji_no']))->find();
					if($read){
						$isread = 1;
					}else{
						$isread = 2;
   ...
   public function paynumPlay(){
	   if(IS_POST){
		   ...
		   $read = M('read_charge')->where(array('user_id'=>$this->user['id'],'yid'=>$yid,'ji_no'=>$jino))->find();
