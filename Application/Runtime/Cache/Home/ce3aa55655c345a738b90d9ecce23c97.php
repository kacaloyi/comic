...
<head>
    ...
    <meta name="description" content="正版小说漫画在线阅读,连载众多优秀原创国漫等海量精彩内容" />
    <!-- 共用引入资源.开始 -->
    <meta charset="UTF-8">
...
<?php if(is_array($bookcate)): foreach($bookcate as $k=>$v): ?><div class="bm-box mt-10">
		...
		<div class="books-row">
			<?php if(is_array($v['list'])): $i = 0; $__LIST__ = $v['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item">
				<a href="<?php echo U('Yook/'.$vo[id]);?>">
					<img class="lazy" src="<?php echo ($vo["cover_pic"]); ?>" data-original="<?php echo ($vo["cover_pic"]); ?>" style="width:210px;height:110px;" />
...
<?php if($mf): ?><div class="bm-box mt-10">
	...
	<div class="books-row">
		<?php if(is_array($mf)): $i = 0; $__LIST__ = $mf;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div style="width:100%;padding-bottom: 20px;border-bottom: 1px solid #ddd;position:relative;">
				...
				<div onclick="location.href=&#39;<?php echo U('Yook/'.$vo['id']);?>&#39;" style="position: absolute;top: 10px;right: 10px;padding: 8px;border: 1px solid #FF9800;border-radius: 5px;color: #FF9800;">立即阅读</div>
			</div><?php endforeach; endif; else: echo "" ;endif; ?>
	</div>
</div>
<?php else: ?>
