...
<p style="text-align:center">境外的漫友请联系<a href="/Member/help.html" class="red">【客服】</a>升级</p>
<div class="recharge-notice">
    <div class="title">充值说明</div>
    <p>1、人民币/书币的汇率为：1元=<?php echo ($_site['rate']); ?>书币</p>
    <p>2、书币属于虚拟商品，一经购买概不退换。</p>
    <p>3、若充值遇到任何问题，可以联系我们的<a href="/Member/help.html" class="red">【客服】</a>。</p>
</div>
...
<script type="text/javascript">
	...
	$(".action .wap-btn").click(function(){
		...
		tips('数据正在请求中，请稍等...');
		$.post("<?php echo U('Member/pay_ajax');?>",{money:fee,vip:vip},function(d){
			...
			if(d){
				if(d.status){
					if(model == 1){
						...
						WeixinJSBridge.invoke(
							...
							function(res){
								...
								if(res.err_msg == "get_brand_wcpay_request:ok"){
									tips('充值成功');
									setTimeout(function(){
										location.href="<?php echo U('Member/my');?>";
									},2000)
						...
						location.href = "<?php echo U('Member/paysApi');?>&qrcode="+d.info.qrcode+"&sn="+d.info.sn;
					}else if(model == 4){
					   	location.href = "<?php echo U('Member/payChinaxingChange');?>&sn="+d.info.sn;
					}
				}else{
					tips(d.info);
				}
