...
<div class="cz_form">
    <div class="tx_j">可提现金额：<span>￥<?php echo ($user['money']); ?></span></div>
...
$('.toggle').click(function(data){
	...
	$.post("<?php echo U('Ucenter/withdraw');?>",{status:status,money:money},function(data){
		$('#show').show();
		$('#hide').hide();
		if(data){
			if(data.status){
...
function withdraw_recode(){
	$.post("<?php echo U('Ucenter/withdraw_recode');?>",{p:p},function(data){
		if(data){
			console.log(data);
			if(data.status){
				$('#recode').html('');
