...
<div class="rt-bar" style="position: absolute">
    <div class="row">
        ...
        <div class="col link">
            <a href="<?php echo U('Mh/'.$jiinfo['mhid']).'?isji=1';?>">全集</a>
        </div>
    </div>
</div>
...
<!-- 	<div class="zshu">
    ...
    <div class="row">
        ...
        <div class="col">
            <a href="<?php echo U('Mh/'.$jiinfo[mhid]).'?isji=1';?>" style="-webkit-tap-highlight-color: rgba(0,0,0,0);">
...
<?php if($user['vip']): else: ?>
     <div class="row">
      <a href="<?php echo U('Member/pay');?>"style="text-align:center;">
 ...
 $before = $jiinfo['before']; if($before <= 1) { $before = 1; } $next = $jiinfo['next']; $money = M('mh_episodes')->where(array('ji_no'=>$next,'mhid'=>$mhinfo['id']))->getField('money'); $money = $money?$money:$_site['jimoney']; $read = M('mh_read')->where(array('ji_no'=>$next,'mhid'=>$mhinfo['id'],'user_id'=>$user['id']))->find(); ?>
            <a href="<?php echo U('Mh/'.$mhid.'/'.$before);?>" onclick="return checkSub();"><i></i>上一话</a>
        </div>
        <div class="item next">
			<?php if($next >= $mhinfo['pay_num'] and $mhinfo['pay_num'] > 0): if($read): ?><a href="<?php echo U('Mh/'.$mhid.'/'.$next).'?sub='.$_GET['sub'];?>" onclick="return checkSub();">下一话<i></i></a>
				<?php else: ?>
					<a href="<?php echo U('Mh/'.$mhid.'/'.$next).'?sub='.$_GET['sub'];?>" onclick="return checkSub();">下一话<i></i></a><?php endif; ?>
			<?php else: ?>
				<a href="<?php echo U('Mh/'.$mhid.'/'.$next).'?sub='.$_GET['sub'];?>" onclick="return checkSub();">下一话<i></i></a><?php endif; ?>
        </div>
    </div>
	...
	function chapter_dianzan(mhid, ji_no) {
		...
		var fun1 = function(data2){
			if(data2.status == 1) {
				...
				alert(data2.info);
				location.href ="<?php echo U('Member/login');?>"; //"index.php?m=Home&c=MhPublic&a=login";
			}
		}
	...
	$(function() {  
		...
		$('.ds li').click(function(){
			if(confirm("确定要打赏么？")){
				...
				$.post("<?php echo U('Member/mxSend');?>",{sid:sid,mxid:mxid,type:"mh"},function(d){
					if(d){
						if(d.status){
							dsHide();
							alert(d.info);
