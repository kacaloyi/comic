...
<script>
	...
	function Audio(){
		...
		if($jino>=$paynum){
			$.post("<?php echo U('paynumPlay');?>",{jino:$jino,vid:"<?php echo ($info["id"]); ?>"},function(d){
				if(d){
					if(d.status){
						if (audio.src != path) {
							audio.src = path;
	...
	function loadData(){
		$.post("<?php echo U('getJino');?>",{page:page,vid:"<?php echo ($info["id"]); ?>"},function(d){
			if(d){
				if(d.status){
					$('#lists').append(d.info);
				}else{
...
<script>
	window.shareData = {
		...
		link: "<?php echo complete_url(U('bookinfo',http_build_query(array_merge(array('uid' => encode($user['id']),'vid'=>$info['id']),$_GET))));?>",
