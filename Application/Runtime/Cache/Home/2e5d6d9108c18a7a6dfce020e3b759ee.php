...
<script type="text/javascript">
    $(function(){
        $(".feedback-box .action a.btn").click(function(){
            ...
            AjaxJson('index.php?m=&c=Mh&a=my_feedbackdo',{content:encodeURIComponent(feedback_content),random:+(new Date)},function(res){
                if(res.status==1){
                    //成功了
                    bh_msg_tips('谢谢，您的反馈我们收到了');
                    setTimeout(function(){
                        that.removeClass('posting');
                        window.location.replace("<?php echo U('Mh/my');?>");
                    },1500);
                    return false;
                }else{
                    //出错了
