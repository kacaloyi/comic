...
<html data-dpr="1" style="font-size: 41.6px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title><?php echo ($info["title"]); ?>无弹窗_<?php echo ($info["title"]); ?>最新章节列表_<?php echo ($_CFG['site']['name']); ?>小说网</title>
    <meta name="keywords" content="<?php echo ($info["title"]); ?>,<?php echo ($info["title"]); ?>最新章节,<?php echo ($info["title"]); ?>无弹窗">
    <meta name="description" content="<?php echo ($info["title"]); ?>无弹窗最新章节由网友提供，《<?php echo ($info["title"]); ?>》情节跌宕起伏、扣人心弦，是一本情节与文笔俱佳的<?php echo ($vo); ?>小说，<?php echo ($_CFG['site']['name']); ?>小说网免费提供一世倾城最新清爽干净的文字章节在线阅读。">
    <!-- 共用引入资源.开始 -->
...
<div class="ac-box mb-tabar" id="chapters" style="display: none;">
    ...
    <?php if(is_array($huas)): $i = 0; $__LIST__ = $huas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $money = M('book_episodes')->where(array('ji_no'=>$vo,'mhid'=>$info['id']))->getField('money'); if(!$money || $money<=0){ $money = $_site['xsmoney']; } $money = intval($money); $read = M('book_read')->where(array('ji_no'=>$vo,'mhid'=>$info['id'],'user_id'=>$user['id']))->find(); ?>
		...
		<?php if($vo >= $info['pay_num'] and $info['pay_num'] > 0): if($read): ?><div class="item">
				<a href="<?php echo U('Book/'.$info['id'].'/'.$vo);?>" class=""><?php echo ($vo); ?>章<span><?php echo ($money); ?>书币</span></a></div>
			<?php else: ?>
				<div class="item lock">
				<a href="<?php echo U('Book/'.$info['id'].'/'.$vo);?>" class=""><?php echo ($vo); ?>章<span><?php echo ($money); ?>书币</span></a></div><?php endif; ?>
		<?php else: ?>
			<div class="item">
			<a href="<?php echo U('Book/'.$info['id'].'/'.$vo);?>" class="" style="text-align:center;"><?php echo ($vo); ?>章<span></span></a></div><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    </div>
    <div class="action">
...
<div class="book-bar">
    <div class="inner">
        ...
        <div class="action">
            <a href="<?php echo U('Book/'.$info['id'].'/1');?>" class="btn">开始阅读</a>
        </div>
    </div>
</div>
