...
<!------------------------------------->
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">转账记录</h1>
            <span class="pagedesc">企业付款记录</span>
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				<thead>
					<tr>
						...
						<th class="head0">用户昵称</th>
						<th class="head0">金额</th>
						<th class="head0">状态</th>
						<th class="head0">支付信息</th>
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
						<?php if($vo['status'] == -1): ?>支付失败
						<?php elseif($vo['status'] == 1): ?>未支付
						<?php elseif($vo['status'] == 2): ?>支付成功
						<?php else: ?>未知<?php endif; ?>
						</td>
						<td><?php echo ($vo["msg"]); ?></td>
						
						<td><?php echo (date("Y-m-d H:i:s",$vo["create_time"])); ?></td>
						<td><?php echo ($vo["op"]); ?></td>
