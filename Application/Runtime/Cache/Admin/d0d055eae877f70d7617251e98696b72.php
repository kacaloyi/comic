			...
			<div class="tableoptions">                    
				<form method="post">
					...
					<select name="action" default="<?php echo ($_GET['action']); ?>" style="width:90px; min-width:0;">
						<option value="">全部</option>
						<?php
 for($i=0;$i<20;$i++){ $action = get_finance_action($i); if(!empty($action) && $action!='未知操作'){ echo '<option value="'.$i.'">'.$action.'</option>'; } } ?>
					</select>
					用户:
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<?php elseif($vo['type'] == 'qimiaobi'): ?>
						旅游基金
						<?php elseif($vo['type'] == 'wxpay'): ?>
						微信支付
