	...
	if($bookname!=""){
    	...
    	$result=$conn->query($stype);
  	 	if($result->num_rows>0){
            ...
            ('$bookname', '$tstype', $send, '$sstype', '$author', '$des', '$litpic', '$litpic', 1, '$status', '$free_type', 1, $pay_num, $reads_mh, $dz_mh, $sc_mh, 1, 1, $time, $time, 1, 0, 0, '$bookname', '$litpic', '$des')";
	...
	if($bookname!=""){
    	...
    	$result=$conn->query($stype);
  	 	if($result->num_rows>0){
			...
			('$bookname', '$tstype', '$sstype',$send, '$author', '$des', '$litpic', '$litpic', 1, '$status', '$free_type', 1, '$pay_num', $reads_book, $dz_book, $sc_book, 1, 1, $time, $time, 0, 0, 0)";
       		$result=$conn->query($booksql);
       		$lasmhid=mysqli_insert_id($conn);
			$jino=$row['episodes']+1;
	  		$before=$jino-1;
